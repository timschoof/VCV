function order=VCVorderWithConstraints(InDir)

% generate a random order of VCVS, subject to constraints which are applied
...
%% code identical to VCVorder up to here

% write out table of the stimuli to be ordered
% This is necessary because there are multiple tokens available for each
% combination of C, V & talker

writetable(struct2table(tmp), 'VCVs2Randomise.txt', 'WriteVariableNames', 0)
[MixStatus, MixOut] =system('Mix AoHL-VCV-constraints.txt');
VCVsRandomised=readtable('VCVsRandomised.txt','ReadVariableNames',false);
order = table2struct(VCVsRandomised);
order=cell2struct(struct2cell(order), {'wave', 'vowel', 'consonant', 'speaker'});


% 
% %% generate a random permutation of the list, 
% % but control for consonants coming too many times in a row
