...
fprintf(fout, 'listener,date,sTime,tracking,trial,targets,SNR,adj_SNR,OutLevelChange,VolumeSettings,manipulation,lateralized,ITD,masker,wave,V,C,response,correct,rTime,rev');
fclose(fout);
...
%     y = [y;y;y;y;y;y;y;y;y;y;y;y;y;y;y;y;y;y]; % for calibration
    ...
    fprintf(fout, '\n%s,%s,%s,%d,%d,%s,%+5.1f,%+5.1f,%+5.1f,%s,%s,%s,%+5.1f,%s,%s,%s,%s,%s,%d,%s', ...
        ListenerName,StartDate,StartTimeString,tracking,trial,TargetDirectory,nominal_SNR_dB,SNR_dB,...
        OutLevelChange,VolumeSettingsFile,itd_invert,lateralize,ITD_us,NoiseFileName,InFile,InFile(1),consonant,response,correct,TimeOfResponse);
...
if ~strcmp(TestType,'fixed')
    ...
    fprintf(fout, '\n%s,%s,%s,%s,%s,%d,%s,%s,%s,%s,%s,%s,%+5.1f,%s', ...
        ...
        TestType,tracking,TargetType,TargetDirectory,NoiseFileName,VolumeSettingsFile,itd_invert,lateralize,ITD_us,VERSION);
